...
public class RangeBomb implements Runnable{
	...
	public RangeBomb(Player1 p1, Player2 p2, ArrayList<ArrayList<String>> mapList) {
		this.p1 = p1;
		this.p2 = p2;
	...
	public void getRender(GraphicsContext g) {
		if(imReady == false) {	//solange er wartet -> mehr oder weniger
			...
			if(explosionTime == true) {
				if(i < 25) {	//25	-> timer einbauen (ganz kurz warten und dann anzeigen, und das ein paar mal machen (20 Schritte oder so)) -> deswegen vlt fehler
					renderExplosion(g);	
					i++;
				}else {
		...
		if(exOb2) {
			if(exOb2Score != 0) {
				if(p1.getScoreMult() == 1) {
					...
					g.setFill(Color.WHITE);
					g.setFont(new Font("", 30));
					g.fillText("+"+exOb2Score*p1.getScoreMult(), x, (y-100)-slideY);
				}	
			}
		}
		if(exUn1) {
		...
		if(exUn2) {
			if(exUn2Score != 0) {
				if(p1.getScoreMult() == 1) {
					...
					g.setFill(Color.WHITE);
					g.setFont(new Font("", 30));
					g.fillText("+"+exUn2Score*p1.getScoreMult(), x, (y+100)-slideY);
				}
			}
		}
		if(exRe1) {
		...
		if(exRe2) {
			if(exRe2Score != 0) {
				if(p1.getScoreMult() == 1) {
					...
					g.setFill(Color.WHITE);
					g.setFont(new Font("", 30));
					g.fillText("+"+exRe2Score*p1.getScoreMult(), x+100, y-slideY);
				}	
			}
		}
		if(exLi1) {
		...
		if(exLi2) {
			if(exLi2Score != 0) {
				if(p1.getScoreMult() == 1) {
					...
					g.setFill(Color.WHITE);
					g.setFont(new Font("", 30));
					g.fillText("+"+exLi1Score*p1.getScoreMult(), x-100, y-slideY);
				}
			}
		}
	}
